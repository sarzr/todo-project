...
const TodoList: React.FC = () => {
  ...
  const todosByPriorities = useMemo(() => {
    return priorities.reduce(
      (acc: Record<string, ITodo[] | undefined>, priority) => {
        acc[priority] = data?.items.filter((el) => el.priorities === priority);
        return acc;
      },
      {}
    );
  ...
  const renderTodoCardsByPriorities = (priority: string) => {
    const todos = todosByPriorities[priority];
    return (
  ...
  return (
    <>
      ...
      {isSuccess && priorities.map((el) => renderTodoCardsByPriorities(el))}
    </>
  );
};
